...
const LocationMap = ({ pickup, delivery }) => {
  ...
  // Function to open the map in the browser
  const openInMaps = (pickup, delivery) => {
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="bg-gray-100 rounded-lg h-64 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-medium text-gray-700">{pickup} <br /> To <br /> {delivery}</p>
          <button 
          onClick={() => openInMaps(pickup, delivery)}
          className="mt-3 px-4 py-2 bg-red-500 text-white text-sm rounded-lg hover:bg-red-600 transition cursor-pointer">
            Open in Maps
          </button>
        </div>
      </div>
